...
// 디버깅 편의용 윈도우 함수
//---------------------------------------------------------------------------
#if defined(TRACE_WIN_REQUIRE_IMPL)
#include <tchar.h> //_vstprintf_s
// 메시지를 출력하고 E_FAIL을 리턴함
...
// Original Version(2007) from (SDK root)\Samples\C++\DXUT\Optional\SDKsound,SDKwavefile.
//---------------------------------------------------------------------------
...
HRESULT CSoundManager::init(HWND hWnd)
{
	...
	if( FAILED( Initialize( hWnd, DSSCL_PRIORITY) ) )
		return TRACE(TEXT("Error initializing DirectSound."));
...
HRESULT CSoundManager::add(LPTSTR filename, int *id)
{
	// Create a sound file
...
HRESULT CSoundManager::play(int id, bool bLooped)
{
...
HRESULT CSoundManager::Initialize( HWND  hWnd, DWORD dwCoopLevel )
{
	...
	if( FAILED( hr = DirectSoundCreate8( NULL, &m_pDS, NULL ) ) )
		return TRACE(hr, TEXT("DirectSoundCreate8"));

	// Set DirectSound coop level
	if( FAILED( hr = m_pDS->SetCooperativeLevel( hWnd, dwCoopLevel ) ) )
		return TRACE(hr, TEXT("SetCooperativeLevel"));
						                       ...
						                       DWORD dwPrimaryBitRate )
{
	...
	if( FAILED( hr = m_pDS->CreateSoundBuffer( &dsbd, &pDSBPrimary, NULL ) ) )
		return TRACE(hr, TEXT("CreateSoundBuffer"));
	...
	if( FAILED( hr = pDSBPrimary->SetFormat(&wfx) ) )
		return TRACE(hr, TEXT("SetFormat"));
...
HRESULT CSoundManager::Get3DListenerInterface( IDirectSound3DListener** ppDSListener )
{
	...
	*ppDSListener = NULL;
	...
	{
		SAFE_RELEASE( pDSBPrimary );
		return TRACE(hr, TEXT("QueryInterface"));
	}
						       ...
						       DWORD dwNumBuffers )
{
	...
	if( FAILED(hr) )
	{
		...
		TRACE(hr, TEXT("CreateSoundBuffer"));
	...
	if( (dwCreationFlags & DSBCAPS_CTRLFX) == 0 )
	{
		for( i=1; i<dwNumBuffers; i++ )
		{
			...
			{
				TRACE(hr, TEXT("DuplicateSoundBuffer"));
				goto LFail;
			}
		}
	}
						                ...
						                HANDLE hNotifyEvent )
{
						                                  ...
						                                  aPosNotify ) ) )
	{
		...
		SAFE_DELETE_ARRAY( aPosNotify );
		return TRACE(hr, TEXT("SetNotificationPositions"));
	}
...
HRESULT CSound::FillBufferWithSound( IDirectSoundBuffer* pDSB, BOOL bRepeatWavIfBufferLarger )
{
	...
	if( FAILED( hr = RestoreBuffer( pDSB, NULL ) ) )
		return TRACE(hr, TEXT("RestoreBuffer"));

	// Lock the buffer down
	if( FAILED( hr = pDSB->Lock( 0, m_dwDSBufferSize,
						         &pDSLockedBuffer, &dwDSLockedBufferSize,
						         NULL, NULL, 0L ) ) )
		return TRACE(hr, TEXT("Lock"));
		...
		if( bRepeatWavIfBufferLarger )
		{
			...
			while( dwReadSoFar < dwDSLockedBufferSize )
			{
				...
				if( FAILED( hr = m_pWaveFile->ResetFile() ) )
					return TRACE(hr, TEXT("ResetFile"));
...
HRESULT CSound::RestoreBuffer( IDirectSoundBuffer* pDSB, BOOL* pbWasRestored )
{
	...
	if( FAILED( hr = pDSB->GetStatus( &dwStatus ) ) )
		return TRACE(hr, TEXT("GetStatus"));
...
HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags, LONG lVolume, LONG lFrequency, LONG lPan )
{
	...
	if( pDSB == NULL )
		return TRACE(TEXT("GetFreeBuffer"));
	...
	if( bRestored )
	{
		...
		if( FAILED( hr = FillBufferWithSound( pDSB, FALSE ) ) )
			return TRACE(hr, TEXT("FillBufferWithSound"));
	}
...
HRESULT CStreamingSound::HandleWaveStreamNotification( BOOL bLoopedPlay )
{
	...
	if( FAILED( hr = m_apDSBuffer[0]->GetCurrentPosition( &dwCurrentPlayPos, NULL ) ) )
		return TRACE(hr, TEXT("GetCurrentPosition"));
...
HRESULT CWaveFile::Open( LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
		...
		if( NULL == m_hmmio )
		{
			...
			if( NULL == ( hResInfo = FindResource( NULL, strFileName, TEXT("WAVE") ) ) )
			{
				if( NULL == ( hResInfo = FindResource( NULL, strFileName, TEXT("WAV") ) ) )
					return TRACE(TEXT("FindResource"));
			}

			if( NULL == ( hResData = LoadResource( GetModuleHandle(NULL), hResInfo ) ) )
				return TRACE(TEXT("LoadResource"));

			if( 0 == ( dwSize = SizeofResource( GetModuleHandle(NULL), hResInfo ) ) )
				return TRACE(TEXT("SizeofResource"));

			if( NULL == ( pvRes = LockResource( hResData ) ) )
				return TRACE(TEXT("LockResource"));

			m_pResourceBuffer = new CHAR[ dwSize ];
			if( m_pResourceBuffer == NULL )
				return TRACE(E_OUTOFMEMORY, TEXT("new"));
	...
	else
	{
		m_hmmio = mmioOpen( strFileName, NULL, MMIO_ALLOCBUF  |
						                          ...
						                          MMIO_CREATE );
		if( NULL == m_hmmio )
			return TRACE(TEXT("mmioOpen"));

		if( FAILED( hr = WriteMMIO( pwfx ) ) )
		{
			mmioClose( m_hmmio, 0 );
			return TRACE(hr, TEXT("WriteMMIO"));
		}
...
HRESULT CWaveFile::ReadMMIO()
{
	...
	if( ( 0 != mmioDescend( m_hmmio, &m_ckRiff, NULL, 0 ) ) )
		return TRACE(TEXT("mmioDescend"));

	// Check to make sure this is a valid wave file
	if( (m_ckRiff.ckid != FOURCC_RIFF) ||
		(m_ckRiff.fccType != mmioFOURCC('W', 'A', 'V', 'E') ) )
		return TRACE(TEXT("mmioFOURCC"));
	...
	// if there are extra parameters at the end, we'll ignore them
	   if( ckIn.cksize < (LONG) sizeof(PCMWAVEFORMAT) )
		   return TRACE(TEXT("sizeof(PCMWAVEFORMAT)"));

	// Read the 'fmt ' chunk into <pcmWaveFormat>.
	if( mmioRead( m_hmmio, (HPSTR) &pcmWaveFormat,
				  sizeof(pcmWaveFormat)) != sizeof(pcmWaveFormat) )
		return TRACE(TEXT("mmioRead"));
	...
	{
		SAFE_DELETE( m_pwfx );
		return TRACE(TEXT("mmioAscend"), E_FAIL );
	}
	...
	else
	{
		...
		else
		{
			...
			if( 0 != mmioCreateChunk( m_hmmio, &m_ck, 0 ) )
				return TRACE(TEXT("mmioCreateChunk"));

			if( 0 != mmioGetInfo( m_hmmio, &m_mmioinfoOut, 0 ) )
				return TRACE(TEXT("mmioGetInfo"));
		}
	}
		...
		for( DWORD cT = 0; cT < cbDataIn; cT++ )
		{
			...
			if( mmioinfoIn.pchNext == mmioinfoIn.pchEndRead )
			{
				if( 0 != mmioAdvance( m_hmmio, &mmioinfoIn, MMIO_READ ) )
					return TRACE(TEXT("mmioAdvance"));

				if( mmioinfoIn.pchNext == mmioinfoIn.pchEndRead )
					return TRACE(TEXT("mmioinfoIn.pchNext"));
			}
